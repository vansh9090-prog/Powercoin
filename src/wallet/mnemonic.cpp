...
namespace powercoin {
    ...
    // BIP39 English wordlist (first 24 words as sample - full list would be 2048 words)
    const std::vector<std::string> ENGLISH_WORDS = {
        "abandon", "ability", "able", "about", "above", "absent", "absorb", "abstract",
        "absurd", "abuse", "access", "accident", "account", "accuse", "achieve", "acid",
        "acoustic", "acquire", "across", "act", "action", "actor", "actress", "actual",
        "adapt", "add", "addict", "address", "adjust", "admit", "adult", "advance",
        "advice", "aerobic", "affair", "afford", "afraid", "africa", "africa", "after",
        ...
        "exact", "example", "exceed", "exchange", "excite", "exclude", "excuse", "execute",
        ...
        "upset", "urban", "urge", "usage", "use", "used", "user", "usual",
    ...
    WordList::WordList(MnemonicLanguage lang) {
        load(lang);
    }

    bool WordList::load(MnemonicLanguage language) {
        switch (language) {
    ...
    bool WordList::contains(const std::string& word) const {
        return wordToIndex.find(word) != wordToIndex.end();
    }

    uint16_t WordList::getIndex(const std::string& word) const {
    ...
    std::string WordList::getWord(uint16_t index) const {
        if (index < words.size()) {
            return words[index];
        }
        return "";
    ...
    std::string WordList::languageToString(MnemonicLanguage language) {
        switch (language) {
            case MnemonicLanguage::ENGLISH: return "english";
            case MnemonicLanguage::CHINESE_SIMPLIFIED: return "chinese_simplified";
            case MnemonicLanguage::CHINESE_TRADITIONAL: return "chinese_traditional";
            case MnemonicLanguage::FRENCH: return "french";
            case MnemonicLanguage::ITALIAN: return "italian";
            case MnemonicLanguage::JAPANESE: return "japanese";
            case MnemonicLanguage::KOREAN: return "korean";
            case MnemonicLanguage::SPANISH: return "spanish";
            case MnemonicLanguage::CZECH: return "czech";
            case MnemonicLanguage::PORTUGUESE: return "portuguese";
            default: return "english";
        }
    }

    MnemonicLanguage WordList::stringToLanguage(const std::string& lang) {
        if (lang == "english") return MnemonicLanguage::ENGLISH;
        if (lang == "chinese_simplified") return MnemonicLanguage::CHINESE_SIMPLIFIED;
        if (lang == "chinese_traditional") return MnemonicLanguage::CHINESE_TRADITIONAL;
        if (lang == "french") return MnemonicLanguage::FRENCH;
        if (lang == "italian") return MnemonicLanguage::ITALIAN;
        if (lang == "japanese") return MnemonicLanguage::JAPANESE;
        if (lang == "korean") return MnemonicLanguage::KOREAN;
        if (lang == "spanish") return MnemonicLanguage::SPANISH;
        if (lang == "czech") return MnemonicLanguage::CZECH;
        if (lang == "portuguese") return MnemonicLanguage::PORTUGUESE;
        return MnemonicLanguage::ENGLISH;
    }

    // ============== MnemonicValidationResult Implementation ==============

    MnemonicValidationResult::MnemonicValidationResult()
        : isValid(false), strength(MnemonicStrength::BITS_128), language(MnemonicLanguage::ENGLISH) {}

    std::string MnemonicValidationResult::toString() const {
        std::stringstream ss;
        ss << "Mnemonic Validation: " << (isValid ? "VALID" : "INVALID") << "\n";
        if (!isValid) {
            ss << "  Error: " << error << "\n";
        } else {
            ss << "  Strength: " << static_cast<int>(strength) << " bits\n";
            ss << "  Words: " << (static_cast<int>(strength) / 32 * 3) << "\n";
            ss << "  Language: " << WordList::languageToString(language) << "\n";
    ...
    Mnemonic::Mnemonic() : strength(MnemonicStrength::BITS_128), language(MnemonicLanguage::ENGLISH) {}

    Mnemonic::Mnemonic(const std::vector<std::string>& w, MnemonicLanguage lang)
        : words(w), language(lang) {
        
        auto result = validate(w, lang);
        if (result.isValid) {
    ...
    Mnemonic::Mnemonic(const std::vector<uint8_t>& ent, MnemonicLanguage lang)
        : entropy(ent), language(lang) {
        
        size_t entropyBits = entropy.size() * 8;
        
        // Determine strength from entropy size
        switch (entropyBits) {
            case 128: strength = MnemonicStrength::BITS_128; break;
            case 160: strength = MnemonicStrength::BITS_160; break;
            case 192: strength = MnemonicStrength::BITS_192; break;
            case 224: strength = MnemonicStrength::BITS_224; break;
            case 256: strength = MnemonicStrength::BITS_256; break;
            default: return;
        }

        // Generate words from entropy
        auto checksum = calculateChecksum(entropy);
        auto indices = entropyToIndices(entropy, checksum);
        
        WordList wordList(language);
        words.clear();
        for (uint16_t index : indices) {
            words.push_back(wordList.getWord(index));
        }
    }

    std::vector<uint8_t> Mnemonic::generateEntropy(MnemonicStrength str) const {
        size_t entropyBytes = static_cast<size_t>(str) / 8;
        return Random::getBytes(entropyBytes);
    }

    std::vector<uint8_t> Mnemonic::calculateChecksum(const std::vector<uint8_t>& ent) const {
        auto hash = SHA256::hash(ent.data(), ent.size());
        
        // Checksum size = entropy size / 32 bits
        size_t checksumBits = ent.size() * 8 / 32;
        size_t checksumBytes = (checksumBits + 7) / 8;
        
        std::vector<uint8_t> checksum(checksumBytes);
        for (size_t i = 0; i < checksumBytes; i++) {
            checksum[i] = hash[i];
        }
    ...
    std::vector<uint16_t> Mnemonic::entropyToIndices(const std::vector<uint8_t>& ent,
                                                      const std::vector<uint8_t>& checksum) const {
        // Combine entropy and checksum bits
        std::vector<uint8_t> combined = ent;
        ...
        for (uint8_t byte : combined) {
            ...
            while (bits >= 11) {
                uint16_t index = (buffer >> (bits - 11)) & 0x7FF;
        ...
        if (bits > 0) {
            uint16_t index = (buffer << (11 - bits)) & 0x7FF;
            indices.push_back(index);
        }
    ...
    std::pair<std::vector<uint8_t>, std::vector<uint8_t>> Mnemonic::indicesToEntropy(
        ...
        for (uint16_t index : indices) {
            ...
            while (bits >= 8) {
                combined.push_back((buffer >> (bits - 8)) & 0xFF);
                bits -= 8;
            }
        }

        // Calculate entropy size (last byte is partial checksum)
        size_t entropyBytes = combined.size() - 1;
        size_t checksumBytes = 1;

        std::vector<uint8_t> entropy(combined.begin(), combined.begin() + entropyBytes);
        std::vector<uint8_t> checksum(combined.begin() + entropyBytes, combined.end());

        return {entropy, checksum};
    }

    Mnemonic Mnemonic::generate(MnemonicStrength strength, MnemonicLanguage language) {
        Mnemonic mnemonic;
        mnemonic.strength = strength;
        mnemonic.language = language;
        mnemonic.entropy = mnemonic.generateEntropy(strength);
        
        auto checksum = mnemonic.calculateChecksum(mnemonic.entropy);
        auto indices = mnemonic.entropyToIndices(mnemonic.entropy, checksum);
        
        WordList wordList(language);
        for (uint16_t index : indices) {
            mnemonic.words.push_back(wordList.getWord(index));
        }

        return mnemonic;
    }

    Mnemonic Mnemonic::fromEntropy(const std::vector<uint8_t>& entropy, MnemonicLanguage language) {
        return Mnemonic(entropy, language);
    }

    Mnemonic Mnemonic::fromWords(const std::vector<std::string>& words, MnemonicLanguage language) {
        return Mnemonic(words, language);
    }

    Mnemonic Mnemonic::fromString(const std::string& phrase, MnemonicLanguage language) {
    ...
    MnemonicValidationResult Mnemonic::validate(const std::string& phrase, MnemonicLanguage language) {
        std::vector<std::string> words;
        std::stringstream ss(phrase);
        std::string word;
        ...
        return validate(words, language);
    }

    MnemonicValidationResult Mnemonic::validate(const std::vector<std::string>& words, MnemonicLanguage language) {
        MnemonicValidationResult result;
        result.language = language;

        // Check word count
        size_t wordCount = words.size();
        size_t expectedChecksumBits = 0;
        
        switch (wordCount) {
            case 12:
                result.strength = MnemonicStrength::BITS_128;
                expectedChecksumBits = 4;
                break;
            case 15:
                result.strength = MnemonicStrength::BITS_160;
                expectedChecksumBits = 5;
                break;
            case 18:
                result.strength = MnemonicStrength::BITS_192;
                expectedChecksumBits = 6;
                break;
            case 21:
                result.strength = MnemonicStrength::BITS_224;
                expectedChecksumBits = 7;
                break;
            case 24:
                result.strength = MnemonicStrength::BITS_256;
                expectedChecksumBits = 8;
                break;
            default:
                result.error = "Invalid word count";
                return result;
        }
        ...
        for (const auto& word : words) {
            if (!wordList.contains(word)) {
                result.error = "Word not in wordlist: " + word;
                return result;
            }
            indices.push_back(wordList.getIndex(word));
        }

        // Convert indices back to entropy and checksum
        auto [entropy, checksum] = Mnemonic().indicesToEntropy(indices);

        // Calculate expected checksum
        auto expectedChecksum = Mnemonic().calculateChecksum(entropy);

        // Verify checksum (only need first expectedChecksumBits)
        bool checksumValid = true;
        size_t checksumBytes = (expectedChecksumBits + 7) / 8;
        
        for (size_t i = 0; i < checksumBytes; i++) {
            uint8_t mask = (i == checksumBytes - 1) ? 
                (0xFF << (8 - (expectedChecksumBits % 8))) & 0xFF : 0xFF;
            
            if ((checksum[i] & mask) != (expectedChecksum[i] & mask)) {
                checksumValid = false;
                break;
            }
        }
        ...
        result.checksum = SHA256::bytesToHash(
            *reinterpret_cast<std::array<uint8_t, 32>*>(expectedChecksum.data())
        ).substr(0, expectedChecksumBits / 4);

        return result;
    }

    std::vector<uint8_t> Mnemonic::generateSeed(const std::string& passphrase) const {
        return BIP39Seed::derive(words, passphrase);
    }

    bool Mnemonic::generateAndStoreSeed(const std::string& passphrase) {
    ...
    bool Mnemonic::isValid() const {
        return !words.empty() && !entropy.empty();
    }
    ...
    WordList Mnemonic::getWordList(MnemonicLanguage language) {
        return WordList(language);
    }

    std::vector<MnemonicLanguage> Mnemonic::getSupportedLanguages() {
        return {
            MnemonicLanguage::ENGLISH,
            MnemonicLanguage::CHINESE_SIMPLIFIED,
            MnemonicLanguage::CHINESE_TRADITIONAL,
    ...
    std::string Mnemonic::languageToString(MnemonicLanguage language) {
        return WordList::languageToString(language);
    }

    MnemonicLanguage Mnemonic::languageFromString(const std::string& name) {
        return WordList::stringToLanguage(name);
    }

    std::string Mnemonic::strengthToString(MnemonicStrength strength) {
        switch (strength) {
            case MnemonicStrength::BITS_128: return "128 bits (12 words)";
            case MnemonicStrength::BITS_160: return "160 bits (15 words)";
            case MnemonicStrength::BITS_192: return "192 bits (18 words)";
            case MnemonicStrength::BITS_224: return "224 bits (21 words)";
            case MnemonicStrength::BITS_256: return "256 bits (24 words)";
            default: return "Unknown";
        }
    }

    size_t Mnemonic::strengthToWordCount(MnemonicStrength strength) {
        return static_cast<size_t>(strength) / 32 * 3;
    }

    MnemonicStrength Mnemonic::wordCountToStrength(size_t wordCount) {
        switch (wordCount) {
            case 12: return MnemonicStrength::BITS_128;
            case 15: return MnemonicStrength::BITS_160;
            case 18: return MnemonicStrength::BITS_192;
            case 21: return MnemonicStrength::BITS_224;
            case 24: return MnemonicStrength::BITS_256;
            default: return MnemonicStrength::BITS_128;
        }
    }

    // ============== BIP39Seed Implementation ==============

    std::vector<uint8_t> BIP39Seed::derive(const std::string& mnemonic, const std::string& passphrase) {
        std::vector<uint8_t> mnemonicBytes(mnemonic.begin(), mnemonic.end());
        
        std::string salt = "mnemonic" + passphrase;
        std::vector<uint8_t> saltBytes(salt.begin(), salt.end());
        
        return pbkdf2(mnemonicBytes, saltBytes, 2048);
    }

    std::vector<uint8_t> BIP39Seed::derive(const std::vector<std::string>& words, const std::string& passphrase) {
        std::string mnemonic;
        for (size_t i = 0; i < words.size(); i++) {
            if (i > 0) mnemonic += " ";
            mnemonic += words[i];
    ...
    std::vector<uint8_t> BIP39Seed::pbkdf2(const std::vector<uint8_t>& mnemonic,
                                            const std::vector<uint8_t>& salt,
                                            uint32_t iterations) {
        return SHA256::pbkdf2(mnemonic.data(), mnemonic.size(),
                               salt.data(), salt.size(),
                               iterations, 64);
    }

    // ============== MnemonicGenerator Implementation ==============

    MnemonicGenerator::MnemonicGenerator()
        : language(MnemonicLanguage::ENGLISH), strength(MnemonicStrength::BITS_128) {}

    void MnemonicGenerator::setProgressCallback(std::function<void(double)> callback) {
    ...
    Mnemonic MnemonicGenerator::generate() {
        ...
        Mnemonic mnemonic = Mnemonic::generate(strength, language);
    ...
    std::string MnemonicGenerator::generatePhrase() {
